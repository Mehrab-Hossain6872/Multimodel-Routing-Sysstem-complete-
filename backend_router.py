...
def get_multimodal_route(graph, start_lat, start_lon, end_lat, end_lon):
    ...
    try:
        ...
        # Call nearest_node without max_distance
        start_node = nearest_node(graph, start_lat, start_lon)
        end_node = nearest_node(graph, end_lat, end_lon)
        
        if start_node is None or end_node is None:
            # Instead of raising an error, return a user-friendly message
            return {
                "error": "No nearby network node found. Please click closer to a road or path.",
                "segments": [],
                "total_time": 0,
                "total_cost": 0
            }
        ...
        try:
            ...
            return {
                "error": "No path found between the selected points. Try clicking closer to a main road or bridge.",
                "segments": [],
                "total_time": 0,
                "total_cost": 0
            }
